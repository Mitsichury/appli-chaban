...
Now you can rename `src/Homepage.css` to `src/Homepage.scss` and run `npm run watch-css`. The watcher will find every Sass file in `src` subdirectories, and create a corresponding CSS file next to it, in our case overwriting `src/Homepage.css`. Since `src/Homepage.js` still imports `src/Homepage.css`, the styles become a part of your application. You can now edit `src/Homepage.scss`, and `src/Homepage.css` will be regenerated.

To share variables between Sass files, you can use Sass imports. For example, `src/Homepage.scss` and other component style files could include `@import "./shared.scss";` with variable definitions.
...
We recommend to put the test files (or `__tests__` folders) next to the code they are testing so that relative imports appear shorter. For example, if `Homepage.test.js` and `Homepage.js` are in the same folder, the test just needs to `import Homepage from './Homepage'` instead of a long relative path. Colocation also helps find tests more quickly in larger projects.
...
This test mounts a component and makes sure that it didn’t throw during rendering. Tests like this provide a lot value with very little effort so they are great as a starting point, and this is the test you will find in `src/Homepage.test.js`.
...
Unlike the previous smoke test using `ReactDOM.render()`, this test only renders `<Homepage>` and doesn’t go deeper. For example, even if `<Homepage>` itself renders a `<Button>` that throws, this test will pass. Shallow rendering is great for isolated unit tests, but you may still want to create some full rendering tests to ensure the components integrate correctly. Enzyme supports [full rendering with `mount()`](http://airbnb.io/enzyme/docs/api/mount.html), and you can also use it for testing state changes and component lifecycle.
